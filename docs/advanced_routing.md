...
```mermaid
graph LR
    GW["istio-ingressgateway"] --> VS["VirtualService: reviews<br/>weighted route to subsets"]
    VS -->|10%| V1["reviews v1"]
    VS -->|10%| V2["reviews v2"]
    VS -->|80%| V3["reviews v3"]
```
...
```mermaid
graph TD
    SVC["reviews Service<br/>(app=reviews)"] --> P1["pod reviews-v1<br/>version=v1"]
    SVC --> P2["pod reviews-v2<br/>version=v2"]
    SVC --> P3["pod reviews-v3<br/>version=v3"]
```
...
```mermaid
graph LR
    DR["DestinationRule: reviews<br/>host: reviews"] --> S1["subset v1<br/>labels.version=v1"]
    DR --> S2["subset v2<br/>labels.version=v2"]
    DR --> S3["subset v3<br/>labels.version=v3"]
```
...
```mermaid
graph LR
    VS["VirtualService: reviews<br/>route: subset + weight"] -->|"weight 10"| V1["reviews v1"]
    VS -->|"weight 10"| V2["reviews v2"]
    VS -->|"weight 80"| V3["reviews v3"]
```
...
graph LR
    ...
    VS -->|"end-user matches → subset v1"| V1["reviews v1"]
    VS -->|"default route (weighted)"| VW["reviews v1 / v2 / v3"]
```
...
```mermaid
graph LR
    U["Request ?test-v2=true"] --> VS["VirtualService: reviews<br/>match: queryParams.test-v2"]
    VS -->|"test-v2 = true → subset v2"| V2["reviews v2"]
    VS -->|"default route"| VD["default subset"]
```
...
```mermaid
graph LR
    U["Request /productpage/v2"] --> VS["VirtualService: reviews<br/>match: uri.regex ^/productpage/v2"]
    VS -->|"uri matches → subset v2"| V2["reviews v2"]
    VS -->|"default route"| VD["default subset"]
```
