...
```mermaid
graph LR
    GW["dispatch-gateway"] --> VS["VirtualService: aidc-dispatch-vs<br/>timeout: 3s (default 15s)"]
    VS -->|"reply &lt; 3s → OK"| SVC["dispatch-svc:444"]
    VS -.->|"reply &gt; 3s → fail fast"| SVC
```
...
```mermaid
graph LR
    GW["dispatch-gateway"] --> VS["VirtualService: aidc-dispatch-vs<br/>retries: attempts 3 · perTryTimeout 3s"]
    VS -->|"retry on failure (up to 3×)"| SVC["dispatch-svc"]
```
