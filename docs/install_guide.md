...
!!! note "About this document"
    This is the installation guide I authored for the GA release of IBM AI Decision Coordination (AIDC) on OpenShift. It is reproduced here as a work sample: internal contacts, cluster URLs, and environment identifiers have been removed or replaced with placeholders.

## 1. Revision history

- 2022-12-08: The documentation has been released in version `1.0.0`.

## 2. Getting started

This guide provides a step-by-step procedure for installing IBM AI Decision Coordination. Before starting the installation, you need to prepare the technical environment (`workstation` or `server`) from where the installation will be executed.

You need to have the two images contained in the `aidc-studio-1.0.0.tar.gz` and `aidc-dispatch-1.0.0.tar.gz` files loaded to a container registry accessible from your target OpenShift environment. Other images obtainable through public repositories might be needed depending on your installation specifics; they are listed below.

### 2.1. Required tools

To be able to perform the next steps, you need the following tools:
...
**Note**: the `libressl` or `openssl` package is only required for generating self-signed certificates.

### 2.2. Resource requirements

To ensure that the product performs well, here are the recommendations:
...
> **Note:** `aidc-studio` should not need to be scaled beyond 2 pods. `aidc-dispatch` supports horizontal scaling: depending on your usage, it can be scaled up to 16 pods to process heavy workloads.
...
These components are covered by the provided Ansible playbook and can be installed with a minimal configuration if you do not have them. You should ensure they are properly and securely set up, managed, and backed up.

The amount of resources required by these components is:
...
If using the provided versions, persistent disks should be provisioned for an equivalent volume.

## 3. Preparing phase

An Ansible playbook is provided for installing IBM AI Decision Coordination on OpenShift, with Keycloak, PostgreSQL, and MongoDB as options. The installation is semi-automated, and the manual steps are described further down the guide.

### 3.1. Terminology

| VARIABLE               | example value    | description                                                  |
| ---------------------- | ---------------- | ------------------------------------------------------------ |
| `<namespace>`         | aidc             | This variable must match the name of your project in OpenShift. |
| `<wildcard_domain_name>` | apps.example.org | This variable must match the domain name used for the apps endpoint on OpenShift. |

### 3.2. Preparing SSL certificates

In a production environment, we recommend encrypting all of the traffic between components by using SSL/TLS with a valid CA (Certificate Authority) recognized in your company.

This part of the guide mostly applies if you want to use the embedded Keycloak or MongoDB deployed by the Ansible playbook.
...
You can follow these steps to generate self-signed certificates for testing purposes.
...
- Generate a CA (Certificate Authority), or use an existing one
...
This should be done for MongoDB, Keycloak, or both, depending on what you install with AIDC.
...
> **Note:** If you use the embedded MongoDB, the `subjectAltName` must also contain `IP:127.0.0.1`. The value is required because a health check is performed by the application itself to verify that there is no issue.

- Concatenate SSL certificates

A `.pem` file containing certificates from the MongoDB is required in all cases. The certificates will come from your existing MongoDB or from the previous instructions.
...
### 3.3. Preparing the OpenShift environment

After generating the SSL certificates, you need to prepare your OpenShift environment before deploying the IBM AI Decision Coordination applications.
...
> **Note**: For the `aidc-mongodb-client-tls` secret, the `mongodb.pem` file is the concatenation of the `mongodb.crt` and `mongodb.key` files.

> **Caution:** In case you bring your own MongoDB, you still need to perform this step for `aidc-studio` and `aidc-dispatch` to be able to establish a connection.

- Create an image pull secret and link it to the `default` service account (or any other way to give access to the AIDC images from your namespace)
...
> **Note**: `aidc-studio` and `aidc-dispatch` container images are not publicly accessible. You need to download them from IBM Passport Advantage and push all images to a private registry accessible from your namespace.
>
...
If your environment is not allowed to make outbound connections, you need to perform some additional steps.

Here is the list of assets used by the Ansible playbook:

- Mirroring container images (if you deploy them with AIDC)
...
Prior to the deployment, you need to download the Ansible playbook through IBM Passport Advantage. Unpack the archive into the folder `${HOME}/aidc_install`, previously created in [3.2. Preparing SSL certificates](#32-preparing-ssl-certificates).

### 4.1. Deployment scenarios

There are multiple scenarios for deploying IBM AI Decision Coordination, depending on which middlewares you need to install. Two files, `config.yml` and `vars/main.yml`, are present in the playbook, to customize before running it.

`config.comments.yml` and `vars/main.comments.yml` contain extra comments that explain the role of each element.
...
You can retrieve the UID range by executing the command `oc describe project <namespace>`.
...
Once you have prepared everything, you can start the deployment by following the steps below:
...
- At this stage, the playbook pauses and lets the user perform the following steps manually.

```shell
TASK [Post-deploy - Phase 1 | Import manually settings for Keycloak] *********
[Post-deploy - Phase 1 | Import manually settings for Keycloak]

Import manually the ${HOME}/aidc_install/playbook/files/keycloak/aidc-realm.json file via the web UI.
...
AIDC needs a Keycloak instance to manage access rights.

It specifically needs an `aidc` realm that defines three clients corresponding to the AIDC main components.

The following steps explain how to set up this realm and extract the resulting configuration files that are to be provided to the last step of the installation.

- Retrieve the URLs of the services
...
- Download the `dispatch-app` and `studio-back` adapter configs

Once created, on the left menu, click on `Clients`, look for the `dispatcher-app` client id, and click on it.

![keycloak_04](imgs/keycloak_04.png)

On the top right, click on `Action` and then click on `Download adapter config`. Copy the details block and paste it into the `playbook/output/aidc/dispatch-app-secret.yaml` file, under the line `kc_config.json: |`.

![keycloak_05](imgs/keycloak_05.png)

Repeat the last two actions for `studio_back`:

Click on `Clients`, look for the `studio_back` client id, and click on it.

On the top right, click on `Action` and then click on `Download adapter config`. Copy the details block and paste it into the `playbook/output/aidc/studio-app-secret.yaml` file, under the line `kc_config.json: |`. **Beware**: there is also a `keycloak_front.json` entry.

- Download the adapter config and configure `studio-front`

Back on the `Clients` page, click on the `studio_front` client id and configure the Valid redirect URIs (ending in `/*`) and Web origins (without trailing `/` or `*`) corresponding to the aidc-studio URL collected above (see the screenshot below), then click on `Save`.

![keycloak_06](imgs/keycloak_06.png)

After saving the changes, click on `Action` and then click on `Download adapter config`. As in the previous steps, copy the details block and paste it into `playbook/output/aidc/studio-app-secret.yaml`, under the line `keycloak_front.json: |`.

![keycloak_07](imgs/keycloak_07.png)

### 4.4. Resuming the Ansible playbook

As mentioned at the end of [4.2. Running the Ansible playbook](#42-running-the-ansible-playbook), the playbook is waiting for you to finish configuring Keycloak.

- Continue the installation

Press `return` to continue the installation.

From this point, the remaining tasks are fully automated. No further action is required from the user.

## 5. Post-installation phase

### 5.1. Get the status of the resources

- Retrieve all resources created by the playbook

You should see something similar to this output (for a full install):
...
service/aidc-keycloak                    LoadBalancer   172.21.24.212    <external-ip>    80:30719/TCP,443:31047/TCP   30m
...
route.route.openshift.io/aidc-dispatch   aidc-dispatch-aidc.<wildcard_domain_name>                   aidc-dispatch   8083    edge/Redirect          None
route.route.openshift.io/aidc-keycloak   aidc-keycloak-aidc.<wildcard_domain_name>                   aidc-keycloak   https   passthrough/Redirect   None
route.route.openshift.io/aidc-studio    aidc-studio-aidc.<wildcard_domain_name>                      aidc-studio     8080    edge/Redirect          None
```

- Check that the two components are up and running
