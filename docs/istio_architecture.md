...
```mermaid
graph LR
    NET["Internet"] --> CLB["Cloud LoadBalancer"]
    CLB --> GW["istio-ingressgateway<br/>(edge Envoy · data plane)"]
    GW --> VS["VirtualService"] --> SVC["productpage Service<br/>(NodePort)"] --> POD["pod + Envoy sidecar<br/>(data plane)"]
    ISTIOD["istiod<br/>(control plane)"] -.->|xDS config| GW
    ISTIOD -.->|xDS config| POD
```
